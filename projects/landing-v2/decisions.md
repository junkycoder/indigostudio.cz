...
### Auto-rozhodnuto orchestrátorem (Fáze 4 pre-launch, 2026-05-08)

- **[2026-05-08] [tech/marketing] Cloudflare Web Analytics snippet → ano, zapnout.** Důvod: marketer default + cookieless (drží CLAUDE.md sekce 2 mantinel), je to first-party-like Cloudflare service. Architect ADR check na úrovni „Cloudflare-native, není to třetí strana v právním smyslu". Aplikace: junior v pre-launch kontrole přidá CF Web Analytics tag do `<head>` všech 5 stránek (`index.html`, `vysledek.html`, `prehled.html`, `ochrana-udaju.html`, `odhlasit-hotovo.html`). Snippet nesahá na cookies, jen agreguje server-side u Cloudflare. **Tip pro Fakana:** v CF dashboardu zapnout „Free" tier, ne Pro.
- **[2026-05-08] [scope/marketing] UTM whitelist v `stripUrl()` + `from=mail` parametr v mailových linkách → odloženo do retra.** Důvod: marketer flagnul jako akci pro juniora, ale není to launch blokátor — současný stripping odstraní utm_*, takže mailové trafiky se neměří per kampaň. Pro bootstrap fázi to není kritické (jeden launch kanál = LinkedIn + Twitter, source je rozeznatelný z referreru). Aplikace: do retra zaznamenat jako kandidáta na drobnou follow-up iteraci.
- **[2026-05-08] [compliance] Retence dat 12 vs. 24 měsíců → 12 měsíců, schváleno legal s podmínkou retro 6m.** Důvod: legal-advisor ve pre-launch checku schválil 12m (datová minimalizace GDPR čl. 5(1)(c)), s podmínkou že po 6 měsících od launche se v retru přehodnotí — pokud business case (nezávazné nabídky vede k uzavření smlouvy v 9-12. měsíci), legal souhlasí s prodloužením na 24m. **Owner potvrdí v delivery** — formálně, ale verdikt je technický kompromis schválený legal+marketer. Fakan jen kývne.
- **[2026-05-08] [compliance] Soubor `legal/consent-versions/v1-2026-05-08.md` → junior dodá v pre-launch.** Důvod: copy.md § 3 slibuje verzování textu souhlasu, legal pre-launch flagnul jako podmínku pro evidenci (audit trail při budoucím přepisu textu). Aplikace: junior vytvoří 5-min markdown s plným zněním v1 textu + datem + odkazem na `decisions.md` pro kontext, kdo a proč to rozhodl.
- **[2026-05-08] [budget] Přestřel agent-času 19 % (~56 h vs. forecast 36–47 h) → akceptuje se.** Důvod: Fakan v Gate 1 delegoval rozpočet na tým agentů („zakázku řeší tým agentů"), runtime cost zůstává $0/měs, žádný externí přečerpaný náklad. Drift je z opravných kruhů (6 fix commitů po code review + tester acceptance) — typický pro první iteraci tohoto typu. Aplikace: zaznamenat do retra, příští iterace by měla mít buffer +20 % v capacity plánu.
