...
const reportUrl  = (env, ev) => `https://${env.PUBLIC_HOST || 'indigostudio.cz'}/audit/${ev.report_token}`;
const optoutUrl  = (env, ev) => `https://${env.PUBLIC_HOST || 'indigostudio.cz'}/odhlasit/${ev.unsub_token}`;
...
export function tplMagicLink({ link, email }) {
  const subject = 'Přihlášení do Indigo Studio';
...
`;
  ...
  const html = `<!doctype html><html lang="cs"><body style="font-family:Georgia,serif;background:#F8F6F2;color:#1E1B4B;margin:0;padding:24px">
<div style="max-width:560px;margin:0 auto;background:#fff;padding:32px;border-radius:6px">
  <h1 style="font-size:26px;margin:0 0 24px">Přihlášení do Indigo Studio</h1>
  ...
  <p style="margin:24px 0">
    <a href="${link}" style="display:inline-block;background:#4F46E5;color:#fff;padding:14px 24px;text-decoration:none;border-radius:4px;font-weight:bold">Přihlásit se</a>
  </p>

  <p style="font-size:14px;color:#6B7280;line-height:1.5">Odkaz platí 30 minut a dá se použít jen jednou.</p>

  <p style="font-size:14px;color:#6B7280;line-height:1.5">Pokud jste o přihlášení nežádali, tenhle e-mail klidně ignorujte — bez kliku se nic nestane.</p>

  <hr style="border:none;border-top:1px solid #DDD9E8;margin:32px 0 16px">
  <p style="font-size:12px;color:#6B7280;line-height:1.5">
    Zaslalo Indigo Studio na ${email} kvůli žádosti o přihlášení. Indigo Studio s.r.o., IČO 14389096.
  </p>
</div></body></html>`;
...
<div style="max-width:560px;margin:0 auto;background:#fff;padding:32px;border-radius:6px">
  ...
  <p style="font-size:14px;color:#6B7280;margin:0 0 24px">${domain}</p>

  ${previewUrl ? `<p style="margin:0 0 24px">
    <img src="${previewUrl}" alt="Náhled navrženého webu" style="display:block;width:100%;height:auto;border:1px solid #DDD9E8;border-radius:6px">
  </p>` : ''}

  <p style="font-size:16px;line-height:1.6">Hotový HTML/CSS soubor i náhled v plné velikosti najdete na:</p>

  <p style="margin:24px 0">
    <a href="${previewPageUrl}" style="display:inline-block;background:#4F46E5;color:#fff;padding:14px 24px;text-decoration:none;border-radius:4px;font-weight:bold">Otevřít náhled</a>
  </p>

  <p style="font-size:14px;color:#6B7280;line-height:1.5">Stačí ho nahrát na Váš hosting. Pokud potřebujete pomoct s nasazením, ozvěte se na <a href="mailto:daniel@indigostudio.cz" style="color:#1E1B4B">daniel@indigostudio.cz</a>.</p>
...
<hr style="border:none;border-top:1px solid #DDD9E8;margin:32px 0 16px">
<p style="font-size:12px;color:#6B7280;line-height:1.5">
  Tenhle e-mail vám jde z Indigo Studia, protože jste si nechali zanalyzovat web ${ev.domain}.
  <a href="${optoutUrl(env, ev)}" style="color:#6B7280">Odhlásit</a> · Indigo Studio s.r.o.
</p>`;

function shell(title, bodyHtml, env, ev) {
  return `<!doctype html><html lang="cs"><body style="font-family:Georgia,serif;background:#F8F6F2;color:#1E1B4B;margin:0;padding:24px">
<div style="max-width:560px;margin:0 auto;background:#fff;padding:32px;border-radius:6px">
${bodyHtml}
${unsubBlock(env, ev)}
</div></body></html>`;
...
<p style="font-size:14px;color:#6B7280;margin:0 0 24px">Audit máme hotový.</p>

<div style="text-align:center;padding:24px;background:#F8F6F2;border-radius:6px;margin:0 0 24px">
  <div style="font-size:64px;font-weight:bold;color:#1E1B4B;line-height:1">${ev.score}<span style="font-size:24px;color:#6B7280">/100</span></div>
  <p style="font-size:16px;font-style:italic;margin:12px 0 0;color:#1E1B4B">${scoreLabel(ev.score)}</p>
</div>

<p style="font-size:16px;line-height:1.6">Pět kategorií, 5–10 konkrétních věcí k řešení a náhled mobilu — vše v reportu:</p>

<p style="margin:24px 0">
  <a href="${reportUrl(env, ev)}" style="display:inline-block;background:#4F46E5;color:#fff;padding:14px 24px;text-decoration:none;border-radius:4px;font-weight:bold">Otevřít report</a>
</p>

<p style="font-size:14px;color:#6B7280">Za 2 dny Vám pošleme konkrétní návrh, co s tím můžeme udělat. Nezávazně.</p>
`, env, ev);
...
<p style="font-size:14px;color:#6B7280;margin:0 0 24px">Audit se automaticky nepodařil.</p>
...
<div style="border-left:3px solid ${accent};padding:16px 20px;margin:0 0 16px;background:#F8F6F2">
  <div style="font-weight:bold;font-size:18px;margin:0 0 4px">${v.title}</div>
  <div style="color:#6B7280;font-size:14px;margin:0 0 12px">${v.price_from?.toLocaleString('cs-CZ')}–${v.price_to?.toLocaleString('cs-CZ')} Kč · ${v.days} dní</div>
  <ul style="margin:0 0 12px;padding-left:18px;font-size:14px;line-height:1.6">${(v.what||[]).map(b => `<li>${b}</li>`).join('')}</ul>
  <div style="font-size:13px;color:#6B7280;font-style:italic">Hodí se: ${v.best_for}</div>
</div>`;

  const html = shell(subject, `
<h1 style="font-size:24px;margin:0 0 8px">${s.headline}</h1>
<p style="font-size:14px;color:#6B7280;margin:0 0 24px">${ev.domain} · skóre ${ev.score}/100</p>
${variantBox(fix, '#84A98C')}
${variantBox(red, '#F59E0B')}
${variantBox(neu, '#4F46E5')}
<p style="font-size:14px;line-height:1.6;margin:24px 0 0"><strong>Co se stane, když s tím nic neuděláte:</strong> ${s.risks}</p>
<p style="margin:24px 0">
  <a href="mailto:daniel@indigostudio.cz?subject=${encodeURIComponent(ev.domain)}" style="display:inline-block;background:#1E1B4B;color:#fff;padding:14px 24px;text-decoration:none;border-radius:4px;font-weight:bold">Odpovědět e-mailem</a>
</p>
`, env, ev);
...
<p style="margin:24px 0">
  <a href="mailto:daniel@indigostudio.cz?subject=${encodeURIComponent('Chci nabídku pro ' + ev.domain)}" style="display:inline-block;background:#4F46E5;color:#fff;padding:14px 24px;text-decoration:none;border-radius:4px;font-weight:bold">Chci nabídku</a>
</p>
<p style="font-size:14px;color:#6B7280">Nebo zavolejte: +420 604 690 539. Beze srandy.</p>
`, env, ev);
  return { subject, html, text: subject };
}
...
<p style="font-size:14px;color:#6B7280">Když je skóre stejné, je nejspíš čas na zásah. Když se zlepšilo, gratulujeme — Vy nebo někdo jiný odvedl práci.</p>
`, env, ev);
  return { subject, html, text: subject };
}
...
<div style="max-width:560px;margin:0 auto;background:#fff;padding:32px;border-radius:6px">
  ...
  <p style="font-size:18px;color:#1E1B4B;margin:0 0 24px">${fqdn}</p>
  ...
  <ul style="font-size:14px;color:#1E1B4B;background:#F8F6F2;padding:14px 32px;border-radius:4px;margin:0 0 16px">
    ${ns.map((h) => `<li style="margin-bottom:4px">${h}</li>`).join('')}
  </ul>` : ''}

  <p style="margin:24px 0">
    <a href="${statusUrl}" style="display:inline-block;background:#4F46E5;color:#fff;padding:14px 24px;text-decoration:none;border-radius:4px;font-weight:bold">Detail objednávky</a>
  </p>

  <p style="font-size:14px;color:#6B7280;line-height:1.5">Pokud potřebujete pomoct s nasazením webu na novou doménu, ozvěte se na <a href="mailto:daniel@indigostudio.cz" style="color:#1E1B4B">daniel@indigostudio.cz</a>.</p>
...
Peníze Vám vrátíme do 3 pracovních dnů. Pokud chcete pomoct vyřešit jiným způsobem, napište na daniel@indigostudio.cz.

— Indigo Studio`;

  const html = `<!doctype html><html lang="cs"><body style="font-family:Georgia,serif;background:#F8F6F2;color:#1E1B4B;margin:0;padding:24px">
<div style="max-width:560px;margin:0 auto;background:#fff;padding:32px;border-radius:6px;border-left:3px solid #4F46E5">
  <h1 style="font-size:24px;margin:0 0 8px">${opLabel} se nepodařil/a</h1>
  <p style="font-size:16px;color:#1E1B4B;margin:0 0 16px">${fqdn}</p>

  ${errorMessage ? `<p style="font-size:14px;background:#F8F6F2;padding:12px 14px;border-radius:4px;color:#1E1B4B">${errorMessage}</p>` : ''}

  <p style="font-size:15px;line-height:1.6">Peníze Vám automaticky vrátíme do 3 pracovních dnů. Pokud chcete vyřešit jiným způsobem, napište nám na <a href="mailto:daniel@indigostudio.cz" style="color:#1E1B4B">daniel@indigostudio.cz</a> a referencujte objednávku <code>${orderId}</code>.</p>

  <hr style="border:none;border-top:1px solid #DDD9E8;margin:32px 0 16px">
  <p style="font-size:12px;color:#6B7280;line-height:1.5">Indigo Studio s.r.o., IČO 14389096.</p>
</div></body></html>`;
