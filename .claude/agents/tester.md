...
Tvoje testování musí skončit v gitu — pass/fail záznam je důkaz, že se to nestalo „nějak".

1. **Test report** patří do `docs/testing/<task-id>.md` — pass/fail per AC, edge case zjištění, bug reporty.
2. **Bug reporty** v jednotném formátu (viz výše) — můžeš je apendovat do test reportu, nebo zakládat samostatný `docs/testing/bugs/<task-id>-<bug>.md`.
3. **Žádné edity produkčního kódu** — fix dělá junior-developer.
4. **Commit po každém ucelení.** Conventional format česky (viz [CLAUDE.md sekce 7.3](../../CLAUDE.md)). Typicky `docs(test): ...`, `docs(bug): ...`.
